...
if ($Action -eq 'Send' -and [string]::IsNullOrWhiteSpace($Variant)) { throw 'Send requires an explicit -Variant; no model effort was selected.' }
...
switch ($Action) {
    ...
    { $_ -in @('Send', 'Reconcile') } {
        ...
        try {
            ...
            $dispatchInput = @{ action = $Action.ToLowerInvariant(); sessionId = $SessionId; directory = $Directory; prompt = $Prompt; variant = $Variant } | ConvertTo-Json -Compress
