...
try {
  ...
  for (const k of ['directory', 'session', 'owner', 'provider', 'model', 'task', 'scope']) if (!o[k]) throw Error('Required: --' + k);
  ...
  try {
    ...
    saveJson(state, { status: 'ready_to_dispatch', codexThreadId: o.owner, sessionId: o.session, sessionTitle: o.title||o.scope, directory, codeDirectory: directory, model: { providerID: o.provider, modelID: o.model }, scope: o.scope, sourceReport: null, currentTaskFile: task, latestReviewReport: null, round: 0, maxRounds, deliveryMode, submittedMessageId: null, lastReviewedAssistantMessageId: null });
