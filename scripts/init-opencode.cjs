...
try {
  ...
  if (Boolean(o.provider) !== Boolean(o.model)) throw Error('Supply both --provider and --model, or omit both to use OpenCode defaults.');
  ...
  try {
    ...
    saveJson(state, { status: 'ready_to_dispatch', codexThreadId: o.owner, sessionId: o.session, sessionTitle: o.title||o.scope, directory, codeDirectory: directory, ...(o.model ? {model: {providerID: o.provider, modelID: o.model}} : {}), scope: o.scope, sourceReport: null, currentTaskFile: task, latestReviewReport: null, round: 0, maxRounds, deliveryMode, submittedMessageId: null, lastReviewedAssistantMessageId: null });
