...
export default function apply(ctx) {
  ...
  ctx.on('session/event', (session, event) => {
    const selected = new Set(['session/header', 'user/message', 'turn/start', 'turn/end', 'step/start', 'llm/retry', 'llm/retry-started', 'tool/call', 'tool/result', 'assistant/message', 'agent/inbox/spliced', 'compaction/start', 'compaction/summary', 'compaction/end']);
    ...
    if (event.type === 'llm/retry') data = { turn: data.turn, step: data.step, retryId: data.retryId,
      retry: data.retry, maxRetries: data.maxRetries, delayMs: data.delayMs,
      code: safeCode(data.failure?.code) };
    if (event.type === 'llm/retry-started') data = { turn: data.turn, step: data.step, retryId: data.retryId, retry: data.retry };
    if (event.type === 'step/start') data = { turn: data.turn, step: data.step };
    if (event.type === 'turn/end') data = { turn: data.turn, reason: { kind: data.reason?.kind,
      ...(data.reason?.error ? { error: { code: safeCode(data.reason.error.code) } } : {}) } };
...
function safeCode(code) { return typeof code === 'string' && /^[A-Z][A-Z0-9_]{0,63}$/.test(code) ? code : 'UNKNOWN'; }
